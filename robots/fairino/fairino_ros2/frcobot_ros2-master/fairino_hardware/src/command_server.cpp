...
robot_command_thread::robot_command_thread(const std::string node_name):rclcpp::Node(node_name,
    ...
    /********************************尝试使用SDK库连接机械臂******************************************/
    _controller_ip = this->get_parameter("controller_ip").as_string();
...
std::string robot_command_thread::FT_SetConfig(std::string para){
    ...
    return std::to_string(_ptr_robot->FT_SetConfig(company,device,softvesion,bus));
}
...
std::string robot_command_thread::FT_Activate(std::string para){
    ...
    return std::to_string(_ptr_robot->FT_Activate(act));
}
...
std::string robot_command_thread::FT_SetZero(std::string para){
    ...
    return std::to_string(_ptr_robot->FT_SetZero(act));
}
...
std::string robot_command_thread::FT_SetRCS(std::string para){
    ...
    return std::to_string(_ptr_robot->FT_SetRCS(ref,coord));
}
...
std::string robot_command_thread::FT_Guard(std::string para){
    ...
    uint8_t select[6];
    for(int i=0;i<6;i++){
        select[i] = std::stoi(list.front());list.pop_front();
    }
    ForceTorque ft;
    ...
    for(int i=0;i<6;i++){
        max_threshold[i] = std::stod(list.front());list.pop_front();
    }
    float min_threshold[6];
    for(int i=0;i<6;i++){
        min_threshold[i] = std::stod(list.front());list.pop_front();
    }

    return std::to_string(_ptr_robot->FT_Guard(flag,sensor_id,select,&ft,max_threshold,min_threshold));
}
...
std::string robot_command_thread::FT_Control(std::string para){
    ...
    float ft_pid[6];
    for(int i=0;i<6;i++){
        ft_pid[i] = std::stod(list.front());list.pop_front();
    }
    ...
    return std::to_string(_ptr_robot->FT_Control(
        flag,sensor_id,select,&ft,ft_pid,adj_sign,ILC_sign,
        max_dis,max_ang,filter_Sign,posAdapt_sign,isNoBlock
    ));
}
...
std::string robot_command_thread::SetForceSensorPayload(std::string para){
    ...
    return std::to_string(_ptr_robot->SetForceSensorPayload(weight));
}
...
std::string robot_command_thread::SetForceSensorPayloadCog(std::string para){
    ...
    return std::to_string(_ptr_robot->SetForceSensorPayloadCog(x,y,z));
}
...
void robot_recv_thread::_state_recv_callback(){
    ...
    static std::vector<char> stream_buff;
   ...
   /* 如果处于重连流程，不需要再读取，直接返回 */
    ...
    while(true){
        int recv_length = recv(_socketfd1,read_buff,sizeof(read_buff),0);
        if(recv_length > 0){
            stream_buff.insert(stream_buff.end(),read_buff,read_buff + recv_length);
            if(stream_buff.size() > 262144){
                stream_buff.erase(stream_buff.begin(),stream_buff.end() - 3);
            }
            continue;
        }
        if(recv_length == 0){
    ...
    const char frame_head[] = "/f/b";
    while(true){
        auto head_it = std::search(
            stream_buff.begin(),
            stream_buff.end(),
            frame_head,
            frame_head + 4
        );
        if(head_it == stream_buff.end()){
            if(stream_buff.size() > 3){
                stream_buff.erase(stream_buff.begin(),stream_buff.end() - 3);
            }
            break;
        }
        if(head_it != stream_buff.begin()){
            stream_buff.erase(stream_buff.begin(),head_it);
        }
        if(stream_buff.size() < 11){
            break;
        }

        int32_t frame_length = 0;
        memcpy(&frame_length,&stream_buff[7],sizeof(frame_length));
        if(frame_length <= 0 || frame_length > 65522){
            RCLCPP_ERROR(rclcpp::get_logger(LOGGER_NAME),msgout[msg_id(search_head_again)]);
            stream_buff.erase(stream_buff.begin());
            continue;
        }

        const int32_t expected_frame_length = static_cast<int32_t>(_CTRL_STATE_SIZE - 14);
        if(frame_length < expected_frame_length){
            std::call_once(oneflag,[&](){
                RCLCPP_WARN(
                    rclcpp::get_logger(LOGGER_NAME),
                    "反馈状态数据帧长度小于预期(actual=%d, expected=%d)，按可用前缀兼容解析。",
                    frame_length,
                    expected_frame_length
                );
            });
        }else if(frame_length > expected_frame_length){
            std::call_once(oneflag,[&](){
                RCLCPP_WARN(
                    rclcpp::get_logger(LOGGER_NAME),
                    "反馈状态数据帧长度大于预期(actual=%d, expected=%d)，按旧结构前缀兼容解析。",
                    frame_length,
                    expected_frame_length
                );
            });
        ...
        const size_t total_length = static_cast<size_t>(frame_length) + 14;
        if(stream_buff.size() < total_length){
            break;
        }

        _CTRL_STATE parsed_state;
        memset(&parsed_state,0,sizeof(parsed_state));
        const size_t copy_len = std::min<size_t>(total_length,_CTRL_STATE_SIZE);
        memcpy(&parsed_state,stream_buff.data(),copy_len);
        if(ctrl_state_store_buff.size() < 3){
            ctrl_state_store_buff.push(parsed_state);//将数据放入队列中
        }else{//如果队列中数据满10个，那么删掉队列头部数据然后队尾再插入数据
            ctrl_state_store_buff.pop();//弹出队首的元素
            ctrl_state_store_buff.push(parsed_state);
        }
        stream_buff.erase(stream_buff.begin(),stream_buff.begin() + total_length);
    }
    ...
    if(ctrl_state_store_buff.empty()){
        int peek_res = recv(_socketfd1,read_buff,1,MSG_PEEK);
        if(peek_res == 0){
            _reconnect_flag.store(true);
            RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME),msgout[msg_id(network_diconnect)]);
            stream_buff.clear();
        ...
        if(peek_res == -1 && errno != EINTR && errno != EWOULDBLOCK && errno != EAGAIN){
            _reconnect_flag.store(true);
            RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME),msgout[msg_id(network_diconnect)]);
            stream_buff.clear();
            return;
