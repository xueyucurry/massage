    ...
    async def call(self, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(self.callback):
                ...
                # 若回调返回的是协程对象，也进行 await（兼容返回协程而非直接值的情况）
...
class McpServer:
    ...
    def add_common_tools(self):
        ...
        # 添加 FAIRINO 轨迹按摩工具（经络检测、轨迹按摩、暂停/继续）。
        # 放在靠前位置，避免 tools/list 分页时被大批通用工具挤到后面。
        try:
            from src.mcp.tools.fairino_massage import get_fairino_massage_manager

            fairino_massage_manager = get_fairino_massage_manager()
            fairino_massage_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
        except Exception as e:
        ...
        # 添加 Runner 工具（运行自定义函数）
        try:
            from src.mcp.tools.runner import get_runner_manager
