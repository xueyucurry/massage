...
from src.utils.text_corrector import correct_text, should_hide_user_input
    ...
    async def on_incoming_json(self, message: Any) -> None:
        ...
        if msg_type in ("tts", "stt"):
            if text := message.get("text"):
                # 如果是用户语音输入(stt)，应用文本纠正
                if msg_type == "stt":
                    config = ConfigManager.get_instance()
                    # 检查是否应该隐藏用户输入
                    if should_hide_user_input(config):
                        # 不显示用户输入
                        return
                    
                    # 应用文本纠正
                    corrected_text, was_corrected = correct_text(text)
                    
                    # 使用纠正后的文本显示
                    await self.display.update_text(corrected_text)
                else:
                    # TTS消息直接显示
